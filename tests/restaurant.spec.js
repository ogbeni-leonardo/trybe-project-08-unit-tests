...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    // ```
    const object = { food: { pizza: 'calabresa' }, drink: { refrigerante: 'coca' } }
    expect(createMenu(object).fetchMenu()).toEqual(object)
    ...
    // ```
    {
      ...
      newFunc.order("agua");
      newFunc.order("sopa");
      newFunc.order("sashimi");
      expect(newFunc.consumption).toEqual(['coxinha', 'agua', 'sopa', 'sashimi'])
    }
    ...
    // ```
    {
      ...
      newFunc.order("agua");
      newFunc.order("coxinha");
      expect(newFunc.consumption).toEqual(['coxinha', 'agua', 'coxinha'])
    }
